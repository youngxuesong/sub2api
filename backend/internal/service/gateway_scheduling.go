...
func (s *GatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, metadataUserID string, sub2apiUserID int64) (*AccountSelectionResult, error) {
	result, err := s.selectAccountWithLoadAwarenessInGroup(ctx, groupID, sessionHash, requestedModel, excludedIDs, metadataUserID, sub2apiUserID)
	if err != nil {
		return nil, err
	}
	if result != nil && groupID != nil {
		result.EffectiveGroupID = *groupID
		result.RouteSourceGroupID = *groupID
		result.EffectiveModel = requestedModel
	}
	return result, nil
}
