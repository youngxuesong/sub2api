...
type APIKeyQuery struct {
	...
	withRouteFailoverTargets *APIKeyRouteFailoverTargetQuery
...
// QueryRouteFailoverTargets chains the current query on the "route_failover_targets" edge.
func (_q *APIKeyQuery) QueryRouteFailoverTargets() *APIKeyRouteFailoverTargetQuery {
	query := (&APIKeyRouteFailoverTargetClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *APIKeyQuery) Clone() *APIKeyQuery {
	...
	return &APIKeyQuery{
		...
		withRouteFailoverTargets: _q.withRouteFailoverTargets.Clone(),
...
// WithRouteFailoverTargets tells the query-builder to eager-load the nodes that are connected to
// the "route_failover_targets" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *APIKeyQuery) WithRouteFailoverTargets(opts ...func(*APIKeyRouteFailoverTargetQuery)) *APIKeyQuery {
	query := (&APIKeyRouteFailoverTargetClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withRouteFailoverTargets = query
	return _q
}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	var (
		...
		loadedTypes = [4]bool{
			...
			_q.withRouteFailoverTargets != nil,
		}
	)
	...
	if query := _q.withRouteFailoverTargets; query != nil {
		if err := _q.loadRouteFailoverTargets(ctx, query, nodes,
			func(n *APIKey) { n.Edges.RouteFailoverTargets = []*APIKeyRouteFailoverTarget{} },
			func(n *APIKey, e *APIKeyRouteFailoverTarget) {
				n.Edges.RouteFailoverTargets = append(n.Edges.RouteFailoverTargets, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *APIKeyQuery) loadRouteFailoverTargets(ctx context.Context, query *APIKeyRouteFailoverTargetQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *APIKeyRouteFailoverTarget)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*APIKey)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(apikeyroutefailovertarget.FieldAPIKeyID)
	}
	query.Where(predicate.APIKeyRouteFailoverTarget(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(apikey.RouteFailoverTargetsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
