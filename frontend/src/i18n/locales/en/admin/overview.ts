export default {
    ...
    groups: {
      ...
      routeFailover: {
        action: 'Failover',
        titleWithGroup: 'Route Failover: {name}',
        enabled: 'Enable failover',
        circuitSettings: 'Retry and Circuit Breaker',
        maxAttempts: 'Maximum attempts per request',
        failureThreshold: 'Failure threshold',
        successThreshold: 'Recovery success threshold',
        windowSeconds: 'Rolling window (seconds)',
        openCooldownSeconds: 'Open cooldown (seconds)',
        halfOpenLeaseSeconds: 'Half-open probe lease (seconds)',
        circuitHint: 'Switching only occurs when the primary has no available accounts or returns a retryable upstream error. Streaming responses are never stitched after output starts.',
        targets: 'Fallback Groups',
        targetsHint: 'Targets are attempted from top to bottom. Only active groups with the same platform and subscription type are shown, up to 9.',
        targetGroup: 'Target group',
        selectTarget: 'Select a fallback group',
        noTargets: 'No fallback groups configured',
        targetEnabled: 'Use for failover',
        moveUp: 'Move up',
        moveDown: 'Move down',
        modelMappings: 'Model mappings (optional)',
        addMapping: 'Add model mapping',
        passThroughModel: 'The original model name is passed through when no mapping is configured',
        sourceModel: 'Requested model',
        targetModel: 'Fallback model',
        unknownGroup: 'Unknown group',
        deletePolicy: 'Delete Policy',
        deleteConfirm: 'Delete this group\'s failover policy? Requests will use only the original group.',
        saved: 'Failover policy saved',
        deleted: 'Failover policy deleted',
        failedToLoad: 'Failed to load failover policy',
        failedToSave: 'Failed to save failover policy',
        failedToDelete: 'Failed to delete failover policy',
        invalidNumbers: 'Attempts, thresholds, and durations must be positive integers. Maximum attempts cannot exceed 10.',
        targetRequired: 'Add at least one fallback group before enabling failover.',
        invalidTarget: 'A fallback group is invalid, duplicated, or no longer matches the platform and subscription type.',
        invalidMapping: 'Both model mapping values are required, and each requested model must be unique.'
      },
      claudeCode: {
