export default {
  ...
  keys: {
    ...
    fallbackGroupsLabel: 'Fallback groups (optional)',
    fallbackGroupsHint: 'Tried in order, up to {count}. The primary group is excluded.',
    addFallbackGroup: 'Add a fallback group',
    searchFallbackGroup: 'Search fallback groups...',
    fallbackGroupsOrder: 'Fallback routing order',
    fallbackGroupsLimitReached: 'Fallback group limit reached',
    moveFallbackUp: 'Move {name} up',
    moveFallbackDown: 'Move {name} down',
    removeFallbackGroup: 'Remove {name}',
    unknownFallbackGroup: 'Group #{id}',
    fallbackRiskTitle: 'Route failover risk',
    fallbackRiskDescription: 'If the primary group is unavailable, requests may switch to a fallback group. Pricing, subscriptions, quotas, and RPM limits then follow the group actually used, and the same session may remain there for up to one hour.',
    fallbackRiskAcknowledge: 'I understand and accept these billing and routing changes',
    fallbackRiskRequired: 'Acknowledge the route failover risk before saving fallback groups',
    usage: 'Usage',
    today: 'Today',
    total: 'Last 30d',
    quota: 'Quota',
