export default {
    ...
    groups: {
      ...
      routeFailover: {
        action: '故障转移',
        titleWithGroup: '路由故障转移：{name}',
        enabled: '启用故障转移',
        circuitSettings: '重试与熔断',
        maxAttempts: '单次请求最多尝试数',
        failureThreshold: '失败熔断阈值',
        successThreshold: '恢复成功阈值',
        windowSeconds: '统计窗口（秒）',
        openCooldownSeconds: '熔断冷却（秒）',
        halfOpenLeaseSeconds: '半开探测租约（秒）',
        circuitHint: '仅在主分组无可用账号或上游返回可重试错误时切换；已开始输出的流式响应不会中途拼接。',
        targets: '备用分组',
        targetsHint: '按从上到下的顺序尝试。只显示已启用、同平台且订阅类型一致的分组，最多 9 个。',
        targetGroup: '目标分组',
        selectTarget: '选择备用分组',
        noTargets: '尚未配置备用分组',
        targetEnabled: '参与切换',
        moveUp: '上移',
        moveDown: '下移',
        modelMappings: '模型映射（可选）',
        addMapping: '添加模型映射',
        passThroughModel: '未配置映射时透传原始模型名',
        sourceModel: '请求模型',
        targetModel: '备用分组模型',
        unknownGroup: '未知分组',
        deletePolicy: '删除策略',
        deleteConfirm: '确定删除此分组的故障转移策略？删除后请求将只使用原分组。',
        saved: '故障转移策略已保存',
        deleted: '故障转移策略已删除',
        failedToLoad: '加载故障转移策略失败',
        failedToSave: '保存故障转移策略失败',
        failedToDelete: '删除故障转移策略失败',
        invalidNumbers: '重试次数、阈值和时间必须填写有效的正整数，最多尝试数不能超过 10。',
        targetRequired: '启用故障转移前至少添加一个备用分组。',
        invalidTarget: '备用分组无效、重复或不再符合平台和订阅类型限制。',
        invalidMapping: '模型映射两端都不能为空，同一请求模型不能重复。'
      },
      claudeCode: {
