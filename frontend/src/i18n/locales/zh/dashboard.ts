export default {
  ...
  keys: {
    ...
    groupRequired: '请选择分组',
    fallbackGroupsLabel: '备用分组（可选）',
    fallbackGroupsHint: '按顺序尝试，最多 {count} 个；主分组不会出现在列表中。',
    addFallbackGroup: '添加备用分组',
    searchFallbackGroup: '搜索备用分组...',
    fallbackGroupsOrder: '备用分组降级顺序',
    fallbackGroupsLimitReached: '已达到备用分组数量上限',
    moveFallbackUp: '上移 {name}',
    moveFallbackDown: '下移 {name}',
    removeFallbackGroup: '移除 {name}',
    unknownFallbackGroup: '分组 #{id}',
    fallbackRiskTitle: '路由降级风险',
    fallbackRiskDescription: '主分组不可用时，请求可能切换到备用分组；切换后按实际分组的价格、订阅、额度和 RPM 规则结算，同一会话最长 1 小时可能继续使用该分组。',
    fallbackRiskAcknowledge: '我已了解并接受上述计费与路由变化',
    fallbackRiskRequired: '选择备用分组后，请先确认路由降级风险',
    usage: '用量',
    today: '今日',
    total: '近30天',
    quota: '额度',
